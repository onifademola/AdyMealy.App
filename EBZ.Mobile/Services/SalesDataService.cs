...
namespace EBZ.Mobile.Services
{
    public class SalesDataService : ISalesDataService
    {
        ...
        public async Task<AspUser> GetUser(string email)
        {
            ...
            var result = await _genericRepository.GetAsync<AspUser>(uri.ToString());
            return result;
        }

        public async Task<String> QueueUser(string userId)
        {
            string uri = Constants.BaseApiUrl + Constants.CheckIn + userId;
            var result = await _genericRepository.GetStringAsync<String>(uri.ToString());
            return result;
        }

        public async Task<String> ServeUser(string userId)
        {
            string uri = Constants.BaseApiUrl + Constants.CheckOut + userId;
            var result = await _genericRepository.GetStringAsync<String>(uri.ToString());
            return result;
        }
