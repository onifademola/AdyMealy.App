...
namespace EBZ.Mobile.ViewModels.Sales
{
    ...
    public class CheckoutPageModel : INotifyPropertyChanged
    {
        #region Fields
        ...
        private void InitializeData()
        {
            ...
            if (Application.Current.Properties.ContainsKey("checkoutCust"))
            {
                _aspUser = (AspUser)Application.Current.Properties["checkoutCust"];
        ...
        private async void PayCommandClicked(object obj)
        {
            ...
            _dialogService.ShowLoading("Serving...");
            var result = await _salesDataService.ServeUser(_aspUser.Id);
            if (result == null)
            {
                _dialogService.HideLoading();
                _dialogService.ShowToast("Sorry! User not served.");
                this.InputSalesPinVerifyColor = "Red";
                return;

            }
            _dialogService.HideLoading();
            _dialogService.ShowToast("User served successfully.");
